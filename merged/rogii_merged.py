...
def tail_features_v3(df: pd.DataFrame, windows: List[int]) -> Dict[str, float]:
...
def typewell_features_v3(path: Path) -> Dict[str, float]:
    defaults = {
        "type_tvt_min": np.nan,
        "type_tvt_max": np.nan,
...
def add_well_features_v3(df: pd.DataFrame, well_id: str, tw_feats: Dict[str, float], windows: List[int]) -> pd.DataFrame:
    out = df.copy()
    ...
    tf = tail_features_v3(out, windows)
...
def load_split_v3(data_dir: Path, split: str, windows: List[int]) -> pd.DataFrame:
    frames = []
    for h_path in sorted((data_dir / split).glob("*__horizontal_well.csv")):
        ...
        frames.append(add_well_features_v3(df, well_id, typewell_features_v3(tw_path), windows))
    if not frames:
...
def feature_columns_v3(train: pd.DataFrame, test: pd.DataFrame, candidate_idx: np.ndarray) -> List[str]:
...
# ---------------------------------------------------------------------------
# v4 / v5 feature engineering
# ---------------------------------------------------------------------------
def tail_stats(hw: pd.DataFrame, kn_idx: np.ndarray, last_idx: int, tail_windows: List[int]) -> Dict[str, float]:
    out: Dict[str, float] = {}
...
def build_well_features(
    ...
    if len(eval_idx) == 0 or len(kn_idx) < 10:
        return None
    last_idx = int(kn_idx[-1])
    stats = tail_stats(hw, kn_idx, last_idx, tail_windows)
...
def build_dataset(
    ...
    for i, h_path in enumerate(files, 1):
        ...
        frame = build_well_features(h_path, tw_path, split, formation_prior, pf_particles, pf_seeds, tail_windows)
...
# model candidates
# ---------------------------------------------------------------------------
def make_model_candidates(seed: int = 42, mode: str = "v5") -> List[Tuple[str, Any]]:
    models: List[Tuple[str, Any]] = []
    # v3 uses fewer models; v4 / v5 use more
    if mode == "v3":
        try:
        ...
        # v4 / v5 candidates (use v5 hyperparams as default)
        try:
            from lightgbm import LGBMRegressor
...
def predict_residual_v3(models: Sequence[Tuple[str, Any]], x: pd.DataFrame) -> np.ndarray:
    if not models:
...
def calibrate_blend(val_df: pd.DataFrame, model_delta: np.ndarray, mode: str = "v5") -> Dict[str, Any]:
    ...
    if mode == "v4":
        ...
        grid_wd = [0.0, 0.05, 0.10, 0.15, 0.20, 0.30]
    else:
...
def train_bundle_v3(
    ...
    fit_idx = sample_idx(fit_idx_all, calibration_fit_rows, seed=42)

    features = feature_columns_v3(train, test, fit_idx)
    print(f"usable features: {len(features)}")

    residual = y - train["last_known_tvt"]
    models = make_model_candidates(seed=42, mode="v3")
    if not models:
        ...
        return ModelBundleV3(
            ...
            metadata={"version": "v3_merged", "mode": "baseline_only", "validation_mae": baseline_mae},
        )
    ...
    val_residual_pred = predict_residual_v3(fitted_calibration, x_val)
    baseline_mae = float(np.mean(np.abs(y.loc[val_idx] - train.loc[val_idx, "last_known_tvt"])))
    alpha, bias, residual_clip, calibrated_mae = calibrate_residual_v3(
        y.loc[val_idx],
    ...
    final_idx = sample_idx(target_idx, max_train_rows, seed=123)
    final_features = feature_columns_v3(train, test, final_idx)
    if final_features != features:
        features = final_features
    final_models = make_model_candidates(seed=123, mode="v3")
    ...
    return ModelBundleV3(fitted_final, features, alpha, bias, residual_clip, metadata)


def predict_with_bundle_v3(bundle: ModelBundleV3, test: pd.DataFrame) -> np.ndarray:
    ...
    residual = predict_residual_v3(bundle.models, x_test)
...
def train_bundle(
    ...
    models = make_model_candidates(seed=42, mode=mode)
    if models:
    ...
    blend = calibrate_blend(train_df.loc[val_idx].reset_index(drop=True), pred_val, mode=mode)

    final_idx = sample_idx(idx_all, max_train_rows, seed=123)
    final_models = make_model_candidates(seed=123, mode=mode)
    if final_models:
...
def make_submission_v3(
    ...
    print("loading train/test files (v3 mode)...")
    windows = [25, 100, 300, 1000]
    train = load_split_v3(data_dir, "train", windows)
    test = load_split_v3(data_dir, "test", windows)
    submission = pd.read_csv(data_dir / "sample_submission.csv")
    print(f"train rows={len(train):,}, test rows={len(test):,}")

    bundle = train_bundle_v3(train, test, max_train_rows, calibration_rows, validation_fraction)
    test = test.copy()
    test["prediction"] = predict_with_bundle_v3(bundle, test)
...
def make_submission_v4_v5(
    ...
    mode: str = "v5",
    tail_windows: List[int] | None = None,
) -> pd.DataFrame:
    if tail_windows is None:
        tail_windows = [15, 30, 50, 100, 250, 500, 1000] if mode == "v5" else [25, 50, 100, 300, 1000]
    t0 = time.time()
    output_path.parent.mkdir(parents=True, exist_ok=True)
    model_dir.mkdir(parents=True, exist_ok=True)
    ...
    test_df = build_dataset(
        ...
        "test",
        formation_prior,
        pf_particles=pf_test_particles,
        pf_seeds=pf_test_seeds,
        tail_windows=tail_windows,
    )

    bundle = train_bundle(train_df, test_df, max_train_rows, calibration_rows, validation_fraction, mode=mode)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="ROGII merged solution (v3 / v4 / v5)")
    parser.add_argument("--mode", choices=["v3", "v4", "v5"], default="v5", help="which pipeline to run")
    ...
    parser.add_argument("--pf-train-particles", type=int, default=None)
    parser.add_argument("--pf-train-seeds", type=int, default=None)
    parser.add_argument("--pf-test-particles", type=int, default=None)
    parser.add_argument("--pf-test-seeds", type=int, default=None)
...
def main() -> None:
    ...
    if args.mode == "v3":
        max_train_rows = args.max_train_rows if args.max_train_rows is not None else 800_000
        calibration_rows = args.calibration_rows if args.calibration_rows is not None else 220_000
        validation_fraction = args.validation_fraction if args.validation_fraction is not None else 0.18
        make_submission_v3(
            data_dir=data_dir,
        ...
        # v4 / v5 defaults
        if args.mode == "v4":
            max_train_rows = args.max_train_rows if args.max_train_rows is not None else 900_000
            calibration_rows = args.calibration_rows if args.calibration_rows is not None else 260_000
            validation_fraction = args.validation_fraction if args.validation_fraction is not None else 0.18
            pf_train_particles = args.pf_train_particles if args.pf_train_particles is not None else 96
            pf_train_seeds = args.pf_train_seeds if args.pf_train_seeds is not None else 2
            pf_test_particles = args.pf_test_particles if args.pf_test_particles is not None else 192
            pf_test_seeds = args.pf_test_seeds if args.pf_test_seeds is not None else 4
        else:
            max_train_rows = args.max_train_rows if args.max_train_rows is not None else 1_000_000
            calibration_rows = args.calibration_rows if args.calibration_rows is not None else 280_000
            validation_fraction = args.validation_fraction if args.validation_fraction is not None else 0.16
            pf_train_particles = args.pf_train_particles if args.pf_train_particles is not None else 128
            pf_train_seeds = args.pf_train_seeds if args.pf_train_seeds is not None else 3
            pf_test_particles = args.pf_test_particles if args.pf_test_particles is not None else 256
            pf_test_seeds = args.pf_test_seeds if args.pf_test_seeds is not None else 5

        make_submission_v4_v5(
            data_dir=data_dir,
            output_path=output_path,
            model_dir=model_dir,
            max_train_rows=max_train_rows,
